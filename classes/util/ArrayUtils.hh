...
class ArrayUtils {
  public static function array2sql(array<mixed> $ar) : string {
    if(!arcnt($ar)) return "";
  ...
  public static function getKey(mixed $ar, mixed $value) : mixed {
  ...
  public static function hasKey(mixed $target, mixed $key) : bool {
    if(is_array($target)) return array_key_exists($key, $target);
    else if($target instanceof Map) return $target->containsKey($key);
